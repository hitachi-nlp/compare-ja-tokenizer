...
from core import NothingPreTokenizer

special_tokens=["[UNK]", "[CLS]", "[SEP]", "[PAD]", "[MASK]"]


def wordpiece(args):
    """Train a WordPiece-based tokenizer without pretokenzier."""
    # init
...
def unigram(args):
    """Train a Unigram-based tokenizer without pretokenizer."""
    # init
...
def bpe(args):
    """Train a BPE-based tokenizer without pretokenizer."""
    # init
    tokenizer = Tokenizer(BPE())
    tokenizer.pre_tokenizer = PreTokenizer.custom(NothingPreTokenizer())
    # set up a trainer
    trainer = BpeTrainer(
        special_tokens=special_tokens, 
        vocab_size=args.vocab_size
...
if __name__ == '__main__':
    ...
    parser = ArgumentParser(
        description="Training a tokenizer without a pre-tokenzier"
    )
    parser.add_argument(
        "--data_path", 
