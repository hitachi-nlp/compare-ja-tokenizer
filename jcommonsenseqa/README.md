...
Here, we explain how to fine-tune our BERT models on the JCommonsenseQA dataset.

## 1. MeCab
```bash
cd ./compare-ja-tokenizer/jcommonsenseqa
...
* The resulting weight files will be stored under `./weights/jcommonsenseqa`. If you want to change the location, you must edit `output_dir` in each shell script.
* Log files will be saved under `./log/jcommonsenseqa`. If you want to change the location, you must edit `--logging_dir` in each shell script.
